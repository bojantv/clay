...
function attachConnection(ctx) {
  ...
  var resumeInterruptedSession = ctx.resumeInterruptedSession;
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    if (active && !autoCreated) {
      ...
      // Sleep/wake recovery: a frozen daemon never sets interruptedByRestart, so
      // the call above is a no-op for that case. Continue an open turn here if the
      // stream watchdog didn't already. Runs after autoResumeRestartSession so it
      // bails when that path already queued a resume (no double-fire).
      if (typeof resumeInterruptedSession === "function") {
        resumeInterruptedSession(active);
      }
