...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    if (active && !autoCreated) {
      ...
      // Auto-continue only when the model was genuinely mid-generation at the
      // restart (restartResumeEligible), not for every session that merely
      // carries the sticky "interrupted by restart" note — AND only if the
      // restart was recent. Reopen a mid-turn session long after the restart
      // and it just shows the note instead of silently firing a turn.
      var RESTART_RESUME_WINDOW_MS = 10 * 60 * 1000;
      var restartRecent = !!active.restartInterruptedAt
        && (Date.now() - active.restartInterruptedAt < RESTART_RESUME_WINDOW_MS);
      if (active.restartResumeEligible && !active.restartAutoContinueQueued && typeof scheduleMessage === "function") {
        if (restartRecent) {
          active.restartAutoContinueQueued = true;
          active.restartResumeEligible = false;
          scheduleMessage(active, "continue", Date.now(), "Resume the work that was interrupted when Clay restarted. Continue from where you left off; do not restart from scratch or re-ask for confirmation.", "↻ Resuming after restart");
        } else {
          // Too long since the restart — drop eligibility, keep the note.
          active.restartResumeEligible = false;
        }
      }
