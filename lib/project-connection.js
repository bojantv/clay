...
var { listProviderRoutes, routeForId, knownModelsForProvider } = require("./provider-routes");
...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    if (initialRoute && initialRoute.provider) {
      var knownInitialModels = knownModelsForProvider(initialRoute.provider);
      if (knownInitialModels.length > initialModels.length) initialModels = knownInitialModels;
    }
    ...
    sendTo(ws, {
      type: "model_info",
      model: (restoredActive && restoredActive.model) || sm.currentModel || "",
    ...
    if (active && !autoCreated) {
      ...
      sendTo(ws, { type: "session_switched", id: active.localId, cliSessionId: active.cliSessionId || null, loop: active.loop || null, vendor: active.vendor || null, providerRouteId: active.providerRouteId || null, automationMode: automationForSession(active, sm.currentPermissionMode || "default", getCodexConfig(sm, active)), permissionMode: active.permissionMode || null, codexApproval: active.codexApproval || null, codexSandbox: active.codexSandbox || null, codexWebSearch: active.codexWebSearch || null, hasHistory: (active.history && active.history.length > 0), capabilities: _vendorCaps, mode: active.mode || "gui", terminalId: typeof active.terminalId === "number" ? active.terminalId : null, runtimeMode: active.runtimeMode || null, runtimeTerminalId: typeof active.runtimeTerminalId === "number" ? active.runtimeTerminalId : null, tuiSuspended: !!active.tuiSuspended });
