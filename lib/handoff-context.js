...
function buildHandoffContextFromHistory(history, options) {
  ...
  if (opts.targetRouteLabel || opts.targetModel) {
    header += "\nIf the user asks what they are using now, answer from the target route/model above, not from earlier transcript text.";
  }
  var footer = "Continue seamlessly from the above. Treat this transcript as prior conversation context for the same Clay chat, even though the underlying CLI session is fresh.";
