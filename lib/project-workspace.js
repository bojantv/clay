...
// the shared terminal manager): one for the main checkout on basePort, plus a
// single shared slot on basePort+1 for whichever worktree a session is editing.
//
...
function attachWorkspace(ctx) {
  ...
  // The requested port: the main checkout asks for basePort; every worktree
  // shares a single requested slot on basePort+1 (the old worktree server is
  // killed first so the replacement can reclaim it). This is only the port we
  // ask for — the dev server may fall back to another free port, in which case
  // spawnDev reconciles devServers[dir].port to the port it actually bound.
  function devPort(aw, basePort) {
    if (!basePort) return basePort;
    return (aw || worktreeMeta) ? basePort + 1 : basePort;
  }
  ...
  function devStatusPayload(session, cb) {
    ...
    // get if started now.
    var port = server ? server.port : (det ? devPort(aw, det.basePort) : null);

    // Is one of OUR dev servers (a different worktree on the shared slot) sitting
    // on this port? Then it's not "external" — it's just a different branch, and
    // Start here will replace it.
    var clayOnPort = false;
    Object.keys(devServers).forEach(function (k) {
      var s = devServers[k];
      if (s && k !== boundAbs && s.port === port && tm.has(s.terminalId)) clayOnPort = true;
    });

    gitw.probePort(port, function (live) {
      // external = something is listening on the port that Clay didn't start
      // (e.g. `yarn dev` in another window) — not one of our own servers.
      var external = !running && live && !!port && !clayOnPort;
  ...
  function buildState(session, cb) {
    ...
    if (det) {
      ...
      var port = devPort(aw, det.basePort);
  ...
  // worktree, or the main checkout). opts.takeover frees the target port first
  // — closing a server Clay owns there and killing any foreign process holding
  // it — so "Re-run here" can reclaim a port from a wrong/stale app (e.g.
  // another project sharing the port, or one started outside Clay).
  function startDevServer(ws, msg, opts) {
    ...
    // main checkout). The main checkout runs on basePort; all worktrees share
    // one slot on basePort+1, so switching branches reuses the same port.
    ...
    var port = devPort(startAw, det.basePort);
    var command = (port ? "PORT=" + port + " " : "") + det.command;

    // Free the port we want to bind. There is only one worktree slot
    // (basePort+1), so any other worktree's server is always killed. On
    // take-over we also close a server Clay owns for this same directory.
    var killed = false;
    Object.keys(devServers).forEach(function (k) {
      var s = devServers[k];
      if (!s) return;
      var sameDir = k === runAbs;
      var otherWorktree = runAbs !== mainAbs && k !== mainAbs && !sameDir;
      if ((opts.takeover && sameDir) || otherWorktree) {
        try { tm.close(s.terminalId); } catch (e) {}
        delete devServers[k];
        killed = true;
      }
    });
    ...
    function spawnDev() {
      // The dev server may bind a different port than the one we requested: when
      // PORT is busy, Vite (and most dev servers) fall back to the next free
      // port and print the URL they actually bound to. Parse that URL from the
      // server's output and reconcile devServers[runAbs].port, otherwise the
      // panel shows the requested port — which sits on "starting" forever
      // because nothing is listening there — instead of the real one.
      var outBuf = "";
    ...
    // Wait for the port to free (after killing a server) so the replacement
    // binds it instead of falling back to the next port.
    function proceed() {
      if ((killed || opts.takeover) && port) waitPortFree(port, 12, spawnDev);
      else spawnDev();
    }
    // Take-over also kills any foreign process listening on the port — one Clay
    // doesn't own, started in another terminal or by another project.
    if (opts.takeover && port) gitw.killPort(port, proceed);
    else proceed();
  }
  ...
  function handleWorkspaceMessage(ws, msg) {
    ...
    if (msg.type === "workspace_dev_restart") {
      // "Re-run here": reclaim the port from whatever is currently on it (a
      // wrong/stale app, or one started outside Clay) and start this project's
      // dev server in its place.
