...
function attachWorkspace(ctx) {
  ...
  // Resolves the enriched item object (async — gh calls run off the event loop).
  function enrichItem(ref, env) {
    var key = ref.slug + "#" + ref.number;
    var cached = itemCache[key];
    if (cached && (now() - cached.at) < ITEM_TTL) return Promise.resolve(mergeRef(cached.data, ref));
    return gitw.fetchItem(cwd, slugRepo(ref.slug), ref.number, env).then(function (data) {
      if (!data) {
        data = {
          number: ref.number,
          type: "issue",
  ...
  function buildState(session, cb) {
    ...
    // Run the independent gh lookups concurrently (each is non-blocking).
    var itemPromises = refs.map(function (ref, i) {
      return i < MAX_ENRICHED ? enrichItem(ref, env) : Promise.resolve(lightItem(ref));
    });
