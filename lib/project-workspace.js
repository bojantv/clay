...
function attachWorkspace(ctx) {
  ...
  function devStatusPayload(session, cb) {
    ...
    if (!det || !det.basePort) return stopped(det ? det.basePort : null);
    gitw.findFreePort(det.basePort, ownLivePorts(boundAbs), stopped);
  }
  ...
  function startDevServer(ws, msg, opts) {
    ...
    // main checkout). Each such directory gets its own independent server.
