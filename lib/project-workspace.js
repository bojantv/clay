...
function attachWorkspace(ctx) {
  ...
  function devStatusPayload(cb) {
    ...
    // When we don't own a terminal, fall back to the port detected from
    // package.json so we can still tell whether a server was started outside
    // Clay (e.g. `yarn dev` in another window).
    var port = devServer ? devServer.port : null;
    var script = devServer ? devServer.script : null;
    if (!port) {
      var det = gitw.detectDev(cwd);
      if (det) { port = desiredPort(det.basePort); script = det.script; }
    }
    gitw.probePort(port, function (live) {
      // external = something is listening on the port, but Clay didn't start it.
  ...
  function handleWorkspaceMessage(ws, msg) {
    ...
    if (msg.type === "workspace_dev_status_get") {
      ...
      // Lets the panel notice servers started/stopped outside Clay.
      devStatusPayload(function (p) { sendTo(ws, p); });
      return true;
    }
