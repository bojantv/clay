...
function attachWorkspace(ctx) {
  ...
  // The main checkout runs on basePort; every worktree shares a single slot on
  // basePort+1. Switching to a different worktree branch reuses that same port
  // (the old worktree server is killed first), so branch and port always match.
