...
function attachWorkspace(ctx) {
  ...
  // A minimal item for a manually-pinned ref that failed enrichment (e.g. a
  // transient gh error). The user explicitly pinned it, so it must stay visible
  // with a working link rather than silently vanishing — unlike auto-detected
  // refs, which are dropped when they don't resolve.
  function pinnedFallback(ref) {
    var slug = ref.slug;
    return {
      number: ref.number,
      slug: slug,
      ...
      title: "",
      state: "",
      url: "https://github.com/" + slug + "/issues/" + ref.number,
      labels: [],
      media: [],
      previewUrl: null,
      pinned: true,
  ...
  function buildState(session, cb) {
    ...
    devStatusPayload(session, function (st) {
      ...
      var itemPromises = refs.map(function (ref, i) {
        ...
        // A pinned ref the user added by hand must never silently disappear, so
        // keep a minimal fallback when enrichment fails or returns nothing.
        return ref.pinned ? p.then(function (item) { return item || pinnedFallback(ref); }) : p;
      });
      Promise.all([
