...
function attachWorkspace(ctx) {
  ...
  function handleWorkspaceMessage(ws, msg) {
    ...
    if (msg.type === "workspace_dev_start") {
      ...
      // Starting a worktree server: there is only one worktree slot (basePort+1).
      // Kill any other worktree's server so the new branch takes that same port.
      var killed = false;
      if (runAbs !== mainAbs) {
        Object.keys(devServers).forEach(function (k) {
          if (k !== runAbs && k !== mainAbs && devServers[k]) {
            try { tm.close(devServers[k].terminalId); } catch (e) {}
            delete devServers[k];
            killed = true;
          }
        });
      ...
      // If we replaced a worktree server, wait for the shared port to free so the
      // new server binds it instead of falling back to the next port.
      if (killed && port) waitPortFree(port, 12, spawnDev);
      else spawnDev();
      return true;
    }
