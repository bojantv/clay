...
function attachAutoLaunch(ctx) {
  ...
  function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      // Issue recipes also dedup against COMPLETED sessions (an issue is never
      // started twice) — unless it bounced: the session finished, the issue
      // progressed forward, and it has since returned to a ready status. In
      // that case issueLaunchState has armed it for exactly one relaunch.
      if (!prKind && tl.findExistingSessionForItem(recipe, item, false)) {
        if (!issueLaunchState.shouldRelaunch(issueKey(recipe, item.number))) {
          skipped.push(item);
          continue;
        }
      }
      ...
      else issueLaunchState.recordLaunch(issueKey(recipe, item.number));
