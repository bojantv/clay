...
var { fetchItems, getPrHead } = require("./project-task-sources");
...
function attachAutoLaunch(ctx) {
  ...
  // Resolve the per-PR pass cap: per-recipe override wins, else the global config.
  ...
  function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      started.push(tl.startSessionForItem(null, recipe, item, itemArgs, null, { auto: true }));
    }
  ...
  function runScheduled(record) {
    var recipes = recipeListFrom(readConfig());
  ...
  function notifyCompleted(session, summary) {
    ...
    if (tl.autoKind === "pr-review" && tl.prKey) {
      try {
        ...
        var headSha = recipe ? getPrHead(cwd, recipe, tl.itemNumber) : "";
