...
function attachAutoLaunch(ctx) {
  ...
  // Persist a new enabled value (merging, never clobbering other config keys),
  // then reconcile the schedule so it applies live without a restart.
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "get_auto_launch") {
      if (sendTo) sendTo(ws, Object.assign({ type: "auto_launch_state" }, getState()));
      return true;
    }
    if (msg.type === "set_auto_launch") {
      var state = setEnabled(!!msg.enabled);
      var payload = Object.assign({ type: "auto_launch_state" }, state);
      if (send) send(payload); else if (sendTo) sendTo(ws, payload);
      return true;
    }
    return false;
  }
