...
function attachAutoLaunch(ctx) {
  ...
  function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      // Dedup: never start a second live session for the same item. Delegates
      // to the task launcher's shared check (single source of truth).
      if (tl.findExistingSessionForItem(recipe, item, prKind)) {
        skipped.push(item);
        continue;
      }
