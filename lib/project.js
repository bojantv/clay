...
function createProjectContext(opts) {
  ...
  var sdk = createSDKBridge({
    ...
    scheduleMessage: function (session, text, resetsAt, promptText, displayLabel) {
      scheduleMessage(session, text, resetsAt, promptText, displayLabel);
    },
  ...
  function scheduleMessage(session, text, resetsAt, promptText, displayLabel) {
  ...
  function autoResumeRestartSession(session) {
    ...
    scheduleMessage(session, "continue", Date.now(), "Resume the work that was interrupted when Clay restarted. Continue from where you left off; do not restart from scratch or re-ask for confirmation.", "↻ Resuming after restart");
  }
