...
function createProjectContext(opts) {
  ...
  var sdk = createSDKBridge({
    ...
    onTurnDone: function (session, preview, fullText) {
      ...
      if (_userMessage && typeof _userMessage.flushQueuedUserMessage === "function") {
        // Defer the flush: this runs inside the finishing turn's try/catch,
        // BEFORE that turn's finally tears down query state and resets
        // isProcessing/messageQueue. Flushing synchronously here starts the
        // next turn and then the finally clobbers it — which made steered
        // messages silently stop. setImmediate lets the finally complete first.
        setImmediate(function () {
          _userMessage.flushQueuedUserMessage(session);
        });
        setTimeout(function () {
          _userMessage.flushQueuedUserMessage(session);
        }, 50);
  ...
  function scheduleMessage(session, text, resetsAt, promptText, displayLabel, opts2) {
    ...
    session.scheduledMessage = {
      ...
      timer: setTimeout(function () {
        ...
        var schedUserMsg = { type: "user_message", text: displayText, _ts: Date.now() };
        ...
        sdk.startQuery(session, sendText, null, ensureProjectAccessForSession(session));
  ...
  // Resume a turn left mid-generation by a sleep/freeze rather than a daemon
  // restart. On macOS sleep the daemon process is only frozen, so no restart
  // happens and interruptedByRestart is never set — autoResumeRestartSession is
  // a no-op for that case. This is the belt-and-braces path for when the stream
  // watchdog (sdk-bridge.js) didn't already re-continue the turn on wake. Shares
  // the same 2-resume cap so it can't loop on a chronically-truncating session.
  function resumeInterruptedSession(session) {
    if (!session) return;
    if (session.isProcessing) return; // turn survived / watchdog already re-running it
    if (session.taskStopRequested) return; // user explicitly stopped
    if (session.scheduledMessage) return; // an auto-continue/resume is already queued
    if (session.restartAutoContinueQueued) return; // restart-resume already claimed this turn
    if (session.destroying) return;
    if (!sm.hasInterruptedTurn(session.history)) return; // turn closed cleanly -> nothing to do
    if ((session._consecutiveAutoResumes || 0) >= 2) return; // respect the shared 2-resume cap (sdk-bridge.js)
    session._consecutiveAutoResumes = (session._consecutiveAutoResumes || 0) + 1;
    scheduleMessage(session, "continue", Date.now(), "Resume the work that was interrupted when the connection dropped. Continue from where you left off; do not restart from scratch or re-ask for confirmation.", "↻ Resuming after wake", { autoAction: true });
  }
  ...
  _userMessage = attachUserMessage({
    ...
    onUserMessageDispatched: function (session, text) {
      if (_taskLauncher && typeof _taskLauncher.handleTaskUserMessageDispatched === "function") {
        _taskLauncher.handleTaskUserMessageDispatched(session, text);
      }
    },
    _loop: _loop,
  ...
  var _connection = attachConnection({
    ...
    resumeInterruptedSession: resumeInterruptedSession,
    _email: _email,
