...
function createProjectContext(opts) {
  ...
  var sdk = createSDKBridge({
    ...
    scheduleMessage: function (session, text, resetsAt, promptText) {
      scheduleMessage(session, text, resetsAt, promptText);
    },
  ...
  function scheduleMessage(session, text, resetsAt, promptText) {
    if (!session || !text || !resetsAt) return;
    // `text` is the visible/recorded turn label and the control keyword (e.g.
    // "continue" for restart recovery). `promptText`, when given, is what the
    // model actually receives — used to send an explicit resume instruction
  ...
  var externalCodexSyncTimer = setInterval(function () {
    ...
    for (var ws of clients) {
      ...
      if (!session || session.vendor === "claude") continue;
