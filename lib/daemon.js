...
var relay = createServer({
  ...
  onListGitAccounts: function () {
    return { ok: true, accounts: gitAccounts.listGitHubAccounts() };
  },
  onGetProjectGitAccount: function (slug) {
    for (var gi = 0; gi < config.projects.length; gi++) {
      if (config.projects[gi].slug === slug) {
        ...
        return { ok: true, slug: slug, account: acct, resolved: resolved };
      }
    }
...
function spawnAndRestart() {
  // Dev mode (config.debug): do NOT self-spawn a replacement. The cli.js dev
  ...
  if (config.debug) {
    console.log("[daemon] Dev mode — restarting via dev watcher (no self-spawn)");
    updateHandoff = true;
    setTimeout(function () { gracefulShutdown(); }, 100);
    return;
  }
