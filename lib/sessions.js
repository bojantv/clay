...
function createSessionManager(opts) {
  ...
  function readCliSessionDescriptor(cliSid) {
    ...
    return { cliSid: cliSid, title: title, createdAt: createdAt, lastActivity: lastActivity };
  }

  // Create a Clay session record from a CLI session descriptor and persist it.
  // Returns the new localId.
  ...
  // ~/.claude/projects/<cwd>/ whose cliSessionId is either tombstoned or has
  // never been adopted. Used by the Import-session picker.
  ...
  // session's localId, or null if the file is missing or unusable.
  function importCliSession(cliSid) {
