...
function createSessionManager(opts) {
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      if (hasInterruptedTurn(session.history)) markRestartInterruptedSession(session);
      ...
      if ((!session.vendor || session.vendor === "claude") && session.cliSessionId && findCodexRolloutByThreadId(session.cliSessionId)) {
      ...
      if (session.interruptedByRestart || (session.vendor || null) !== (m.vendor || null) || (session.cliSessionId || null) !== (m.cliSessionId || null)) {
  ...
  try {
    tombstones.prune(function (id) {
      ...
      return !!findCodexRolloutByThreadId(id);
    });
  } catch (e) {}
