...
function createSessionManager(opts) {
  ...
  function markRestartInterruptedSession(session) {
    ...
    var interruptedTs = Date.now();
    // When the interruption happened (≈ daemon load time). Used to recency-gate
    // auto-continue: a mid-turn session only auto-resumes if reopened soon after
    // the restart; reopen it much later and it just shows the note.
    session.restartInterruptedAt = interruptedTs;
  ...
  function recoverMissingHandoffContext(history) {
    ...
    for (var si = switchIndex + 1; si < history.length; si++) {
      if (history[si] && history[si].type === "session_id") {
        sessionIdIndex = si;
        break;
      }
    }
    if (sessionIdIndex < 0) return null;
    return buildHandoffContextFromHistory(history.slice(switchIndex + 1, sessionIdIndex), {
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      if (m.interruptedByRestart || hasUncontinuedRestartInterruption(session.history)) session.interruptedByRestart = true;
      if (hasInterruptedTurn(session.history)) {
        ...
        if (!session.restartInterruptedAt) session.restartInterruptedAt = Date.now();
      }
      ...
      if (!session.handoffContext && !session.handoffContextRecovered && session.vendor === "codex") {
        ...
        if (recoveredHandoffContext) {
          ...
          session.handoffContextTurnsRemaining = 4;
      ...
      if (loaded[i].migrated || session.interruptedByRestart || (session.vendor || null) !== (m.vendor || null) || (session.cliSessionId || null) !== (m.cliSessionId || null)) {
  ...
  // Returns the new localId. Vendor comes from the descriptor; Codex imports
  // land in gui mode (Codex has no TUI), Claude in tui mode.
  ...
  function adoptOrphanedCliSessions() {
    ...
    sessions.forEach(function (s) {
      if (s.cliSessionId) { knownCliIds.add(s.cliSessionId); if (s.hidden) hiddenKnownIds.add(s.cliSessionId); }
      if (s.storageId) { knownCliIds.add(s.storageId); if (s.hidden) hiddenKnownIds.add(s.storageId); }
    });
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (knownCliIds.has(cliSid)) {
        // Instrumentation: a rollout that maps to an already-hidden session must
        // never reappear. If this ever logs, the hidden session is being shadowed.
        if (hiddenKnownIds.has(cliSid)) {
          resurrectAttempts++;
          console.warn("[sessions][unhide-watch] skipped re-adopting rollout " + cliSid + " — already tracked by a HIDDEN session (correctly skipped).");
        }
        continue;
      }
  ...
  function listAdoptableCliSessions() {
    var knownCliIds = new Set();
    var hiddenCliIds = new Set();
    sessions.forEach(function (s) {
      if (!s.cliSessionId) return;
      if (s.hidden) hiddenCliIds.add(s.cliSessionId);
      else knownCliIds.add(s.cliSessionId);
    });
  ...
  function switchSession(localId, targetWs, transform) {
    ...
    // Persist lastViewedAt lazily: saveSessionFile rewrites the entire history,
    // so throttle disk writes to avoid O(history) synchronous IO on every focus
    // change. The in-memory value is always current; only the persisted copy
    // lags by at most the throttle window (used solely for restore ordering).
