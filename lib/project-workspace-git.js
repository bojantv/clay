...
// Run a gh command and JSON-parse stdout, returning null on any failure.
function ghJson(cwd, args, env) {
  try {
...
function getBoardUrl(cwd, repo, env) {
  ...
  var data = ghJson(cwd, ["api", "graphql", "-f", "query=" + q, "-F", "owner=" + repo.owner, "-F", "name=" + repo.repo], env);
  try {
...
function fetchItem(cwd, repo, number, env) {
  ...
  var data = ghJson(cwd, ["api", "repos/" + repo.slug + "/issues/" + number], env);
  if (!data) return null;
  var isPr = !!data.pull_request;
  var allText = data.body || "";
  var media = extractMedia(data.body || "");
  var comments = ghJson(cwd, ["api", "repos/" + repo.slug + "/issues/" + number + "/comments", "--paginate"], env);
