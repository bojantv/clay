...
function startCopilotSession(connection, capabilities, opts) {
  opts = opts || {};
  var base = { cwd: opts.cwd, mcpServers: opts.mcpServers || [] };
  var sessionId = opts.sessionId || null;
  if (sessionId && capabilities && capabilities.sessionCapabilities && capabilities.sessionCapabilities.resume) {
    return connection.resumeSession(Object.assign({}, base, { sessionId: sessionId }));
  }
  if (sessionId && capabilities && capabilities.loadSession) {
    return connection.loadSession(Object.assign({}, base, { sessionId: sessionId }));
  }
