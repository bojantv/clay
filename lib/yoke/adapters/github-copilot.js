...
// Detect a Copilot authorization failure surfaced as an agent message. Kept
// conservative to avoid false positives on normal model output: matches the
// exact known phrase, or any text mentioning both "not authorized" and
// "copilot" (covers reworded/localized variants without being over-broad).
...
function createCopilotQueryHandle(opts) {
  ...
  async function runPrompt(text, images) {
    ...
    pushEvent({ yokeType: "turn_start" });
    try {
      var promptBlocks = [{ type: "text", text: runtimeMetadataPrompt(model, actualModel, modelVerificationSource) + "\n\n" + (text || "") }];
      if (images && images.length > 0) {
        for (var i = 0; i < images.length; i++) {
          if (images[i] && images[i].data && images[i].mediaType) {
            promptBlocks.push({ type: "image", data: images[i].data, mimeType: images[i].mediaType });
          }
        }
      }
      ...
      if (pendingPrompts.length > 0 && !closed) {
        ...
        runPrompt(next.text, next.images).catch(function(err) { queue.fail(err); });
      }
    }
  }

  if (opts.prompt) {
    runPrompt(opts.prompt, opts.images || null).catch(function(err) { queue.fail(err); });
  }
  ...
  return {
    ...
    pushMessage: function(text, images) {
      ...
      else runPrompt(text || "", images || null).catch(function(err) { queue.fail(err); });
    },
