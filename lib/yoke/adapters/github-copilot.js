...
function runtimeMetadataPrompt(model) {
  ...
  return "[Clay runtime metadata]\n" +
    ...
    "Model family: " + family + ".\n" +
    "This metadata describes the current runtime for this turn and overrides any older transcript or handoff text.\n" +
    "If the user asks what provider, route, or model is active, answer from this metadata. Do not introspect your own system prompt, speculate about another hidden model, or repeat a different model from earlier conversation text.\n" +
...
function createCopilotQueryHandle(opts) {
  ...
  var model = resolveKnownCopilotModel(opts.model || "auto");
  ...
  async function applyModelConfig(options) {
    if (!connection || !sessionId || !model || model === "default") return;
    var option = modelConfigOption(options);
    if (!option) return;
    modelConfigId = option.id || "model";
    var resolvedModel = resolveModelOptionValue(option, model);
    if (option.currentValue === model || canonicalModelId(option.currentValue) === canonicalModelId(model)) {
      model = option.currentValue;
      rememberActualModel({ model: model, source: "config" });
      return;
    }
    if (!resolvedModel) {
      modelErrorText = "GitHub Copilot does not expose model '" + model + "' for this account.";
      return;
    }
    model = resolvedModel;
  ...
  async function runPrompt(text) {
    ...
    try {
      var result = await connection.prompt({
        sessionId: sessionId,
        prompt: [{ type: "text", text: runtimeMetadataPrompt(model) + "\n\n" + (text || "") }],
      });
  ...
  return {
    ...
    setModel: function(nextModel) {
      model = resolveKnownCopilotModel(nextModel || model);
