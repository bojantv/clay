...
function flattenEvent(notification, state) {
  ...
  if (method === "thread/tokenUsage/updated") {
    ...
    // One-time raw dump to confirm live field casing (totalTokens vs
    // total_tokens, modelContextWindow vs ...) for the context-window guard.
    if (!state._tokenUsageShapeLogged) {
      state._tokenUsageShapeLogged = true;
      try { console.log("[yoke/codex] tokenUsage payload shape:", JSON.stringify(tu)); } catch (e) {}
    }
    if (tu && tu.total) {
      ...
      var usedTokens = tu.total.totalTokens || tu.total.total_tokens || 0;
