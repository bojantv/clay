...
function buildAutoRecipe(cfg) {
  var filter = {
    state: "open",
    assigned: cfg.assigned === "any" ? "any" : "me",
  };
...
function buildWebsitePrompt(cfg, launchUrl, token) {
  var board = cfg.board || {};
  var options = asArray(board.options);
  var lines = [];
  ...
  lines.push("- Body: `{ \"token\": \"<token>\", \"recipe\": \"" + cfg.recipeId + "-manual\", \"issue\": <number>, \"vendor\": \"claude\" }`");
  lines.push("");
