...
import { handleInputSync, autoResize, builtinCommands, setScheduleBtnDisabled, getInputDraft, restoreInputDraft } from './input.js';
...
import { isProjectSettingsOpen, handleInstructionsRead, handleInstructionsWrite, handleProjectEnv, handleProjectEnvSaved, handleProjectSharedEnv, handleProjectSharedEnvSaved, handleProjectOwnerChanged, handleDashboardConfig, handleDashboardCommandResult, handleDashboardCommandUpdateResult } from './project-settings.js';
import { updateSettingsModels, updateSettingsStats, updateDaemonConfig, handleSetPinResult, handleKeepAwakeChanged, handleAutoContinueChanged, handleRestartResult, handleShutdownResult, handleSharedEnv, handleSharedEnvSaved, handleGlobalClaudeMdRead, handleGlobalClaudeMdWrite, handleSettingsModels } from './server-settings.js';
...
// --- App module imports ---
import { scrollToBottom, addToMessages, addUserMessage, addSystemMessage, removeOptimisticUserMessage, removeMatePreThinking, appendDelta, finalizeAssistantBlock, addConflictMessage, addContextOverflowMessage, showSuggestionChips, armStickyBottom, getPrependAnchor, providerLabel } from './app-rendering.js';
...
import { handleQueuedUserMessage, removeQueuedUserMessage, clearQueuedUserMessages } from './queued-messages.js';
...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      case "cli_session_list":
        handleCliSessionList(msg.sessions || []);
        break;
        ...
        if (msg.sessions && msg.sessions.length > 0) {
          ...
          for (var _si = 0; _si < msg.sessions.length; _si++) {
            ...
            if (_listedSession && _listedSession.vendor) {
              ...
              if (_vendorOverrides[_listedSession.id]) {
                delete _vendorOverrides[_listedSession.id];
                _changedVendorOverrides = true;
              }
            }
          }
      ...
      case "session_switched":
        ...
        if (_prevSid) {
          var _outgoingDraft = getInputDraft ? getInputDraft() : { text: inputEl.value || "" };
          var _hasDraft = !!(_outgoingDraft &&
            ((_outgoingDraft.text && _outgoingDraft.text.length > 0) ||
              (_outgoingDraft.images && _outgoingDraft.images.length > 0) ||
              (_outgoingDraft.pastes && _outgoingDraft.pastes.length > 0) ||
              (_outgoingDraft.files && _outgoingDraft.files.length > 0)));
          if (_hasDraft) {
            store.get('sessionDrafts')[_prevSid] = _outgoingDraft;
          } else {
        ...
        var _sessionSwitchUpdate = { activeSessionId: msg.id, cliSessionId: msg.cliSessionId || null, vendorCapabilities: msg.capabilities || {}, sessionIsProcessing: !!msg.isProcessing, activeSessionMode: _effectiveMode, activeTerminalId: _effectiveTerminalId, sessionHasHistory: !!msg.hasHistory, currentProviderRouteId: msg.providerRouteId || null, requestedModel: msg.requestedModel || "", verifiedModel: msg.verifiedModel || "", modelVerificationSource: msg.modelVerificationSource || "" };
        ...
        var draft = store.get('sessionDrafts')[store.get('activeSessionId')] || null;
        ...
        var queuedForSession = Array.isArray(msg.queuedUserMessages) ? msg.queuedUserMessages : [];
        for (var _qmi = 0; _qmi < queuedForSession.length; _qmi++) {
          handleQueuedUserMessage(queuedForSession[_qmi]);
        }
      ...
      case "error":
        if (String(msg.text || "").trim() === "unknown") break;
        removeMatePreThinking();
        setActivity(null);
        stopThinking();
        markAllToolsDone();
