...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "auto_launch_state":
        var _alToggle = document.getElementById("us-auto-launch");
        if (_alToggle) _alToggle.checked = !!msg.enabled;
        var _alRecipe = document.getElementById("us-auto-launch-recipe");
        if (_alRecipe && Array.isArray(msg.recipes)) {
          ...
          for (var _ali = 0; _ali < msg.recipes.length; _ali++) {
            ...
            _alOpt.value = msg.recipes[_ali];
            _alOpt.textContent = msg.recipes[_ali];
            _alRecipe.appendChild(_alOpt);
          }
          if (msg.recipeId) _alRecipe.value = msg.recipeId;
        }
        var _alCron = document.getElementById("us-auto-launch-cron");
        if (_alCron && msg.cron) _alCron.value = msg.cron;
        var _alVendor = document.getElementById("us-auto-launch-vendor");
        if (_alVendor && msg.vendorWeights) {
          ...
          var _alLbl = document.getElementById("us-auto-launch-vendor-label");
