...
import { updateSettingsModels, updateSettingsStats, updateDaemonConfig, handleSetPinResult, handleKeepAwakeChanged, handleAutoContinueChanged, handleFollowUpBehaviorChanged, handleRestartResult, handleShutdownResult, handleSharedEnv, handleSharedEnvSaved, handleGlobalClaudeMdRead, handleGlobalClaudeMdWrite, handleSettingsModels } from './server-settings.js';
...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "model_info": {
        ...
        if (_modelRouteId) _miUpdate.currentProviderRouteId = _modelRouteId;
        else if (msg.providerRouteId) _miUpdate.currentProviderRouteId = msg.providerRouteId;
        store.set(_miUpdate);
        if (msg.vendor) {
      ...
      case "follow_up_behavior_changed":
        handleFollowUpBehaviorChanged(msg);
        break;
