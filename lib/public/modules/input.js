...
export function sendMessage(options) {
  ...
  if (scheduleDelayMs > 0) {
    ...
    ctx.ws.send(JSON.stringify({ type: "schedule_message", text: text || "", resetsAt: resetsAt, sessionId: store.get("activeSessionId") || null }));
    clearScheduleDelay();
    ctx.inputEl.value = "";
    sendInputSync();
    clearPendingImages();
  ...
  // Always render an optimistic bubble so the sender sees their message in the
  // thread immediately. When the message will be queued (session still
  // processing), it renders as a dimmed "pending" bubble and is reconciled to a
  // solid bubble once it actually runs. See ensurePendingQueuedBubble().
  ctx.addUserMessage(text, images.length > 0 ? images : null, pastes.length > 0 ? pastes : null, null, null, { optimisticId: clientMessageId, pendingQueued: willQueue });
  ...
  document.addEventListener("paste", function (e) {
    ...
    if (!found) {
      ...
      if (pastedText && pastedText.length >= 500) {
        e.preventDefault();
        var fallbackPreview = pastedText.substring(0, 50).replace(/\n/g, " ");
        var preview = meaningfulTextTitle(pastedText, 70) || fallbackPreview;
        if (preview === fallbackPreview && pastedText.length > 50) preview += "...";
