...
export function initTheme() {
  ...
  // Apply wide view state from localStorage. Only apply when there is an
  // explicit cached value — otherwise wait for the server preference to
  // arrive so we don't flash Channel before switching to Bubble.
  try {
    var savedLayout = localStorage.getItem(WIDE_KEY);
    if (savedLayout !== null) {
      document.body.classList.toggle("wide-view", savedLayout !== "bubble");
    }
  } catch (e) {}
