...
function tickRateLimitUsage() {
  ...
  // Stop ticking once every tracked reset time has expired, otherwise the
  // 30s interval would keep firing forever with nothing left to update.
  var stateForVendor = rateLimitResetState[store.get('currentVendor') || "claude"] || {};
  var types = ["five_hour", "seven_day", "seven_day_opus", "seven_day_sonnet"];
  var live = 0;
  for (var i = 0; i < types.length; i++) {
    if (stateForVendor[types[i]] && formatResetTime(stateForVendor[types[i]].resetsAt)) live++;
  }
  if (live === 0 && rateLimitTickTimer) {
...
function renderRateLimitUsageLink() {
  ...
  var types = ["five_hour", "seven_day", "seven_day_opus", "seven_day_sonnet"];
  for (var i = 0; i < types.length; i++) {
    var entry = stateForVendor[types[i]];
    if (!entry || !entry.resetsAt) continue;
    var timeStr = formatResetTime(entry.resetsAt);
    if (!timeStr) { delete stateForVendor[types[i]]; continue; }
    var pctStr = formatUtilization(entry.utilization);
    var label = rateLimitTypeShortLabel(types[i]);
    parts.push(label + (pctStr ? " " + pctStr : "") + " resets " + timeStr);
  }
...
export function updateRateLimitUsage(msg) {
  ...
  var types = ["five_hour", "seven_day", "seven_day_opus", "seven_day_sonnet"];
  for (var i = 0; i < types.length; i++) {
    if (stateForVendor[types[i]] && formatResetTime(stateForVendor[types[i]].resetsAt)) parts.push(types[i]);
  }
