...
export function addUserMessage(text, images, pastes, fromUserId, fromUserName, opts) {
  ...
  if (opts.pendingQueued) markBubblePendingQueued(div, contentWrap);
...
// --- Pending queued message bubbles ---
// A queued message (sent while the session is still processing) renders as a
// dimmed "pending" bubble in the thread so the sender sees it immediately,
// instead of it only living in the queued-message bar. It is tagged with the
// server-assigned queueId so it can be removed when the queue entry is
// cleared, and reconciled to a solid bubble (by clientMessageId) once the
// message actually runs — see addUserMessage().

function markBubblePendingQueued(div, contentWrap) {
  div.classList.add("msg-user-pending");
  div.dataset.pendingQueued = "1";
  if (!contentWrap.querySelector(".msg-user-queued-status")) {
    var status = document.createElement("div");
    status.className = "msg-user-queued-status";
    status.innerHTML = iconHtml("clock") + "<span>Queued · sends after the current turn</span>";
    contentWrap.appendChild(status);
  }
}

function findUserMessageByQueueId(queueId) {
  if (!queueId) return null;
  var messagesEl = getMessagesEl();
  if (!messagesEl) return null;
  var items = messagesEl.querySelectorAll('.msg-user[data-queue-id]');
  for (var i = items.length - 1; i >= 0; i--) {
    if (items[i].dataset.queueId === queueId) return items[i];
  }
  return null;
}

// Ensure a pending bubble exists for a queued item. Promotes the optimistic
// bubble created at send time (matched by clientMessageId) by tagging it with
// the queueId; otherwise renders a fresh pending bubble (e.g. queued from
// another device/user, or restored from server state on reconnect).
export function ensurePendingQueuedBubble(item) {
  if (!item) return;
  var queueId = item.queueId || "";
  var clientMessageId = item.clientMessageId || "";
  if (queueId && findUserMessageByQueueId(queueId)) return;
  var optimistic = clientMessageId ? findUserMessageByClientId(clientMessageId, false) : null;
  if (optimistic) {
    if (queueId) optimistic.dataset.queueId = queueId;
    var contentWrap = optimistic.querySelector(".dm-bubble-content");
    if (contentWrap) markBubblePendingQueued(optimistic, contentWrap);
    refreshIcons();
    return;
  }
  addUserMessage(item.text || "", item.images || null, item.pastes || null, item.from || null, item.fromName || null, { clientMessageId: clientMessageId, queueId: queueId, pendingQueued: true });
}

// Remove a pending bubble by queueId. Guards on the pending flag so a solid
// (already-run) bubble is never removed.
export function removePendingQueuedBubble(queueId) {
  if (!queueId) return;
  var messagesEl = getMessagesEl();
  if (!messagesEl) return;
  var items = messagesEl.querySelectorAll('.msg-user[data-queue-id]');
  for (var i = items.length - 1; i >= 0; i--) {
    if (items[i].dataset.pendingQueued === "1" && items[i].dataset.queueId === queueId) {
      items[i].remove();
    }
  }
}

export function clearPendingQueuedBubbles() {
  var messagesEl = getMessagesEl();
  if (!messagesEl) return;
  var items = messagesEl.querySelectorAll('.msg-user[data-pending-queued="1"]');
  for (var i = items.length - 1; i >= 0; i--) items[i].remove();
}

// Reconcile the set of pending bubbles to a full server-provided list (used on
// reconnect / session state sync). Removes pending bubbles no longer queued and
// adds any missing ones.
export function syncPendingQueuedBubbles(items) {
  var messagesEl = getMessagesEl();
  if (!messagesEl) return;
  var keep = {};
  var list = Array.isArray(items) ? items : [];
  for (var i = 0; i < list.length; i++) {
    if (list[i] && list[i].queueId) keep[list[i].queueId] = true;
  }
  var existing = messagesEl.querySelectorAll('.msg-user[data-pending-queued="1"]');
  for (var j = existing.length - 1; j >= 0; j--) {
    var qid = existing[j].dataset.queueId || "";
    if (!qid || !keep[qid]) existing[j].remove();
  }
  for (var k = 0; k < list.length; k++) ensurePendingQueuedBubble(list[k]);
}
