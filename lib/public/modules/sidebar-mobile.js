...
function renderSheetSessions(listEl) {
  ...
  function buildProjectChip(p, isWorktree, parentSlug) {
    ...
    // collapse state with the desktop icon strip).
    if (isWorktree && isWtCollapsed(parentSlug)) chip.classList.add("wt-hidden");
      ...
      if (worktrees.length > 0) {
        ...
        toggle.addEventListener("click", function (e) {
          ...
          for (var wc = 0; wc < wtChips.length; wc++) {
            wtChips[wc].classList.toggle("wt-hidden", nowCollapsed);
          }
        });
