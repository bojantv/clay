...
function createMobileAutoLaunchRow() {
  ...
  if (activity.startedToday > 0) {
    var started = document.createElement("span");
    started.className = "mobile-autolaunch-activity started";
    started.innerHTML = iconHtml("play") + String(activity.startedToday);
  ...
  row.addEventListener("click", function () {
    closeMobileSheet();
    setTimeout(function () {
      var proj = currentProjectForSettings();
      if (proj) openProjectSettings(getCachedCurrentSlug(), proj, "automation");
    }, 250);
  });
