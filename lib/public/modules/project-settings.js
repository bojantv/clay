  ...
  if (alToggle) {
    alToggle.addEventListener("change", function () {
      if (ctx.ws && ctx.ws.readyState === 1) {
        ctx.ws.send(JSON.stringify({ type: "set_auto_launch", enabled: this.checked }));
  ...
  if (alPrFix) {
    alPrFix.addEventListener("change", function () {
      ...
      if (this.checked && alToggle && !alToggle.checked) {
        ...
        if (ctx.ws && ctx.ws.readyState === 1) {
          ctx.ws.send(JSON.stringify({ type: "set_auto_launch", enabled: true }));
        }
        showToast("Auto-start assigned issues on");
      }
    });
  }
