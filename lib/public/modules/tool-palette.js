...
var SESSION_TOOLS = [
  ...
  { id: "dashboard-btn",           icon: "layout-dashboard", label: "Dashboard",     url: "http://localhost:8765/outstanding-issues.html", projects: ["webapp"] },
];
...
// Resolve a tool URL against the host the page is currently served from.
// Tool URLs are stored with a loopback host (e.g. http://localhost:8765/...),
// which only works when Clay is opened on the same machine. When Clay is
// reached over the network (e.g. a Tailscale IP from a phone), rewrite the
// loopback host to the current hostname so the dashboard — served on the same
// host but its own port — stays reachable. The port and path are preserved.
