...
function showClaudeStartMenu(anchorBtn) {
  ...
  importItem.title = "Pick a CLI session (~/.claude/projects/) to bring into Clay";
...
// Reads /lib/project-sessions.js list_cli_sessions response, shows a list of
// CLI sessions in ~/.claude/projects/<cwd>/ that Clay isn't tracking (deleted
// + never-adopted). User picks one or more; we send import_cli_session per
// click and the session_list broadcast re-renders the sidebar.
...
function openImportSessionPicker() {
  ...
  modal.style.cssText = "background:var(--bg-elev,#1a1a1f);color:var(--text,#fff);border:1px solid var(--border,#333);border-radius:10px;width:520px;max-width:90vw;max-height:80vh;display:flex;flex-direction:column;overflow:hidden";

  var header = document.createElement("div");
  header.style.cssText = "padding:14px 16px;border-bottom:1px solid var(--border,#333);display:flex;align-items:center;justify-content:space-between";
  header.innerHTML = '<strong>Import session from CLI</strong>';
  var closeBtn = document.createElement("button");
  closeBtn.textContent = "×";
  closeBtn.style.cssText = "background:none;border:none;color:var(--text,#fff);font-size:22px;cursor:pointer;padding:0 4px";
  closeBtn.addEventListener("click", closeImportSessionPicker);
  header.appendChild(closeBtn);
  modal.appendChild(header);

  var searchWrap = document.createElement("div");
  searchWrap.style.cssText = "padding:8px 10px;border-bottom:1px solid var(--border,#333)";
  var searchInput = document.createElement("input");
  searchInput.type = "text";
  searchInput.placeholder = "Search sessions…";
  searchInput.className = "import-session-search";
  searchInput.style.cssText = "width:100%;box-sizing:border-box;background:var(--bg,#0e0e12);color:var(--text,#fff);border:1px solid var(--border,#333);border-radius:6px;padding:7px 10px;font:inherit;outline:none";
...
export function handleCliSessionList(sessions) {
  ...
  if (!sessions || sessions.length === 0) {
    body.style.cssText += ";color:var(--text-secondary,#aaa);text-align:center;padding:24px";
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    row.type = "button";
    row.style.cssText = "display:block;width:100%;text-align:left;background:none;border:none;border-bottom:1px solid var(--border,#222);color:var(--text,#fff);padding:10px 12px;cursor:pointer;font:inherit";
    row.addEventListener("mouseover", function () { this.style.background = "var(--bg-hover,#26262e)"; });
    ...
    var meta = (s.tombstoned ? "deleted • " : "") + date;
    row.dataset.searchText = ((s.title || "") + " " + meta).toLowerCase();
    row.innerHTML =
      '<div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' +
        escapeHtml(s.title || "(untitled)") +
      '</div>' +
      '<div style="font-size:11px;color:var(--text-secondary,#888);margin-top:2px">' +
        escapeHtml(meta) +
      '</div>';

    (function (cliSessionId, btn) {
      btn.addEventListener("click", function () {
        if (getWs() && store.get('connected')) {
          getWs().send(JSON.stringify({ type: "import_cli_session", cliSessionId: cliSessionId }));
        }
        btn.disabled = true;
