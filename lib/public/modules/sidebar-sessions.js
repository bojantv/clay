...
var COUNTDOWN_IMMINENT_SEC = 180; // last 3 minutes: switch to the prominent look

function formatCountdown(sec) {
  if (sec <= COUNTDOWN_IMMINENT_SEC) {
...
function updateCountdowns() {
  ...
  for (var i = 0; i < upcoming.length; i++) {
    ...
    var imminent = remaining <= COUNTDOWN_IMMINENT_SEC;
    var timeStr = formatCountdown(remaining);
    var itemClass = "session-countdown-item" + (imminent ? " imminent" : "");
    // Only tint with the schedule's color once it's imminent; subtle otherwise.
    var colorStyle = (imminent && u.color) ? " style=\"border-left-color:" + u.color + "\"" : "";
    html += '<div class="' + itemClass + '"' + colorStyle + '>';
