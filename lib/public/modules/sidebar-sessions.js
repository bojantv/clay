...
function getRoutesForSessionMenu(currentVendor, currentRouteId) {
  ...
  return [
    ...
    },
    {
      ...
      enabled: resolvedRouteId !== "claude-github-copilot" && installed.indexOf("github-copilot") !== -1,
    ...
    },
    {
      ...
      enabled: resolvedRouteId !== "codex-github-copilot" && installed.indexOf("github-copilot") !== -1,
      installed: installed.indexOf("github-copilot") !== -1,
      setup: "Install GitHub Copilot CLI, then run copilot login.",
    },
  ];
...
function showCodexStartMenu(anchorBtn) {
  ...
  importItem.title = "Pick a CLI session to bring into Clay";
...
function openImportSessionPicker(vendorFilter) {
  ...
  var headerLabel = importPickerVendor === "codex" ? "Import Codex session" : importPickerVendor === "claude" ? "Import Claude session" : "Import session from CLI";
  ...
  for (var i = 0; i < filtered.length; i++) {
    ...
    var badge = importPickerVendor
      ...
      : '<span style="display:inline-block;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;padding:1px 6px;border-radius:4px;margin-right:6px;background:' +
        (vendor === "codex" ? "#2d4a6b;color:#9fd0ff" : "#3a3a45;color:#cfcfd6") + '">' + escapeHtml(vendor) + '</span>';
    row.innerHTML =
...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  for (var ri = 0; ri < routes.length; ri++) {
    ...
    handoffItem.innerHTML = iconHtml("arrow-right-left") + " <span>Continue in " + escapeHtml(route.label || vendorLabel(route.vendor)) + "</span>";
    ...
    handoffItem.addEventListener("click", (function(routeForClick) {
      return function(e) {
        ...
        closeSessionCtxMenu();
        if (getWs() && store.get('connected')) {
          getWs().send(JSON.stringify({ type: "handoff_session", sessionId: sessionId, targetVendor: routeForClick.vendor, targetRouteId: routeForClick.id }));
        }
      };
    })(route));
...
function renderSessionItem(s) {
  ...
  var routeFamily = providerShortName(sessionVendor, s.providerRouteId || null).toLowerCase();
  var dotVendor = routeFamily === "codex" ? "codex" : "claude";
  var vendorDotClass = "session-vendor-dot " + dotVendor + (s.isProcessing ? " processing" : "");
  textHtml += '<span class="' + vendorDotClass + '" title="' + providerShortName(sessionVendor, s.providerRouteId || null) + ' session"></span>';
