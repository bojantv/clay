...
var autoLaunchActivity = { events: [], seenAt: 0, counts: { started: 0, completed: 0 } };
...
function updateCountdowns() {
  ...
  for (var i = 0; i < upcoming.length; i++) {
    ...
    if (u.id === AUTOLAUNCH_REC_ID) {
      var c = autoLaunchActivity.counts || {};
      if (c.started > 0) actBadges += '<span class="session-countdown-act started" title="' + c.started + ' session(s) started since you last looked">' + iconHtml("play") + c.started + '</span>';
      if (c.completed > 0) actBadges += '<span class="session-countdown-act done" title="' + c.completed + ' session(s) completed since you last looked">' + iconHtml("check") + c.completed + '</span>';
      if ((autoLaunchActivity.events || []).length > 0) clickable = ' data-act="1" title="Click to see what auto-launch did"';
    }
...
export function setAutoLaunchActivity(data) {
  autoLaunchActivity = {
    ...
    counts: (data && data.counts) || { started: 0, completed: 0 },
  };
  updateCountdowns();
}
...
function showAutoLaunchActivityPopover(anchor) {
  ...
  if (events.length === 0) {
    ...
    for (var i = 0; i < events.length && i < 30; i++) {
      var ev = events[i];
  ...
  // Acknowledge: clears the unseen badge counts (server broadcasts the reset).
  if (getWs() && store.get('connected')) getWs().send(JSON.stringify({ type: "ack_auto_launch_activity" }));
}
