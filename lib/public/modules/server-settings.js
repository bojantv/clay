...
var FOLLOW_UP_OPTIONS = [
  { value: "queue", label: "Queue", desc: "Send while active creates a queued prompt with a Steer now action." },
  { value: "steer", label: "Steer", desc: "Send while active immediately adds your message to the in-flight response." },
];
var VENDOR_SETUP = {
  claude: {
...
function renderFollowUpBehavior() {
  renderOptionList("ss-follow-up-list", FOLLOW_UP_OPTIONS, ctx.followUpBehavior || "queue", function (value) {
    ctx.followUpBehavior = value === "steer" ? "steer" : "queue";
    ssSendMsg("set_server_follow_up_behavior", { behavior: ctx.followUpBehavior });
    renderFollowUpBehavior();
  });
}
...
export function updateDaemonConfig(config) {
  ctx.followUpBehavior = config.defaultFollowUpBehavior === "steer" ? "steer" : "queue";
  renderFollowUpBehavior();
...
export function handleFollowUpBehaviorChanged(msg) {
  if (!ctx) return;
  ctx.followUpBehavior = msg.behavior === "steer" ? "steer" : "queue";
  renderFollowUpBehavior();
}
