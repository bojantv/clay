...
export function initUserSettings(appCtx) {
  ...
  // Auto-start assigned issues: project-scoped automation. State round-trips
  // over the project WS (`auto_launch_state`); the server persists it to
  // .clay/tasks/config.json and reconciles the schedule live.
  var autoLaunchToggle = document.getElementById('us-auto-launch');
  if (autoLaunchToggle) {
    var wsAL = getWs();
    if (wsAL && wsAL.readyState === 1) {
      wsAL.send(JSON.stringify({ type: 'get_auto_launch' }));
    }
    autoLaunchToggle.addEventListener('change', function () {
      var ws = getWs();
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: 'set_auto_launch', enabled: this.checked }));
        showToast(this.checked ? 'Auto-start assigned issues on' : 'Auto-start assigned issues off');
      }
    });
  }

  // Agent split slider: live label "X% Claude · Y% Codex".
  function updateAutoLaunchVendorLabel(claudePct) {
    var lbl = document.getElementById('us-auto-launch-vendor-label');
    if (!lbl) return;
    if (!Number.isFinite(claudePct)) claudePct = 60;
    lbl.textContent = claudePct + '% Claude · ' + (100 - claudePct) + '% Codex';
  }
  var autoLaunchVendor = document.getElementById('us-auto-launch-vendor');
  if (autoLaunchVendor) {
    autoLaunchVendor.addEventListener('input', function () {
      updateAutoLaunchVendorLabel(parseInt(this.value, 10));
    });
  }

  // Recipe + cron + agent split editor (saved together via the Save button).
  var autoLaunchSaveBtn = document.getElementById('us-auto-launch-save');
  if (autoLaunchSaveBtn) {
    autoLaunchSaveBtn.addEventListener('click', function () {
      var ws = getWs();
      if (!ws || ws.readyState !== 1) return;
      var recipeEl = document.getElementById('us-auto-launch-recipe');
      var cronEl = document.getElementById('us-auto-launch-cron');
      var vendorEl = document.getElementById('us-auto-launch-vendor');
      var cronVal = cronEl ? cronEl.value.trim() : '';
      var statusEl = document.getElementById('us-auto-launch-status');
      ...
      var msg = {
        ...
        recipeId: recipeEl ? recipeEl.value : undefined,
