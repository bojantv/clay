...
// The per-project workflow doc (localAIConfig/TRIAGE.local.md) that recipes
// inject into the sessions they start. Edited here over the same fs_read /
// fs_write plumbing the Instructions tab uses for CLAUDE.md.
var COOKBOOK_PATH = "localAIConfig/TRIAGE.local.md";

function loadCookbook() {
  var status = el("tsi-cookbook-status");
  if (status) status.textContent = "Loading…";
  wsSend({ type: "fs_read", path: COOKBOOK_PATH });
}
...
function saveCookbook() {
  ...
  wsSend({ type: "fs_write", path: COOKBOOK_PATH, content: editor.value });
}

export function isCookbookPath(p) {
  return p === COOKBOOK_PATH;
}
...
export function handleCookbookRead(msg) {
  ...
  if (msg.error) {
    editor.value = "";
    if (status) status.textContent = "No cookbook yet — Save to create " + COOKBOOK_PATH + ".";
  } else {
