...
function configChipDisplayName(vendor, routeId, model, models) {
  ...
  if (vendor === "github-copilot" && routeId) {
    var routeName = providerLabel(vendor, routeId).replace("GitHub Copilot", "Copilot");
...
export function initPanels() {
  ...
  function getProviderRoutesForMenu() {
    ...
    return [
      ...
      { id: "claude-github-copilot", vendor: "github-copilot", label: "Claude via GitHub Copilot", description: "Fallback route for Claude-family models through GitHub Copilot CLI.", executable: true, enabled: installed.indexOf("github-copilot") !== -1, setup: "Install GitHub Copilot CLI, then run copilot login." },
      { id: "codex-github-copilot", vendor: "github-copilot", label: "Codex via GitHub Copilot", description: "Fallback route for Codex/GPT coding models through GitHub Copilot CLI.", executable: true, enabled: installed.indexOf("github-copilot") !== -1, setup: "Install GitHub Copilot CLI, then run copilot login." },
    ];
  }
  ...
  if (providerRouteMenu) {
    providerRouteMenu.addEventListener("click", function (ev) {
      ...
      var ws = getWs();
      if (ws) ws.send(JSON.stringify({ type: "handoff_session", targetVendor: route.vendor, targetRouteId: route.id }));
...
export function updateConfigChip() {
  ...
  var routeText = providerLabel(vendor, routeId);
  configChipLabel.textContent = chipText;
  configChip.title = routeText + (s.currentModel ? " · " + modelDisplayName(s.currentModel, s.currentModels) : "");
