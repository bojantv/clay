...
import { ensurePendingQueuedBubble, removePendingQueuedBubble, clearPendingQueuedBubbles, syncPendingQueuedBubbles } from './app-rendering.js';
...
export function handleQueuedUserMessage(msg) {
  ...
  for (var i = 0; i < queuedItems.length; i++) {
    if (queuedItems[i].queueId === msg.queueId) {
      ...
      ensurePendingQueuedBubble(item);
      return;
    }
  }
...
export function setQueuedUserMessages(items) {
  ...
  renderQueuedMessages();
  syncPendingQueuedBubbles(queuedItems);
}
...
export function removeQueuedUserMessage(queueId) {
  ...
  renderQueuedMessages();
  removePendingQueuedBubble(queueId);
}

export function clearQueuedUserMessages() {
  queuedItems = [];
  renderQueuedMessages();
  clearPendingQueuedBubbles();
}
