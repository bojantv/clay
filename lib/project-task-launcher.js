...
function attachTaskLauncher(ctx) {
  ...
  function completionDirective(completion) {
    ...
    lines.push("Only finish if the work is genuinely complete — verify the required changes are done (and committed/pushed if the workflow calls for it). When it truly is, end your final message with the exact marker:");
    lines.push("");
    lines.push(marker);
    lines.push("");
    if (needs) {
  ...
  function launchExternal(body, user) {
    ...
    // Don't start the same item twice — return the existing session if one is
    // already in the list (live-only for PR reviews, forever for issues).
    var existing = findExistingSessionForItem(recipe, items[0], isPrReviewKind(recipe));
