...
function attachTaskLauncher(ctx) {
  ...
  function handleTaskTurnDone(session, preview, fullText) {
    ...
    if (!closeAfterNextTurn && text.indexOf(marker) === -1) return;
  ...
  function handleTaskUserMessage(ws, msg) {
    if (msg.type !== "message") return false;
    var session = getSessionForWs ? getSessionForWs(ws) : null;
    ...
    if (!matchesCompletionTrigger(completion, msg.text || "")) return false;
    session.taskLauncher.closeAfterNextTurn = true;
    session.taskLauncher.closeTriggerText = msg.text || "";
    sm.saveSessionFile(session);
    return false;
  }
  ...
  return {
    ...
    handleTaskUserMessage: handleTaskUserMessage,
