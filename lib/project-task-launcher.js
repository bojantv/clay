...
function attachTaskLauncher(ctx) {
  ...
  function handleTaskTurnDone(session, preview, fullText) {
    ...
    if (session.taskLauncher.autoLaunch) {
      ...
      if (needsMarker && turnText.indexOf(needsMarker) !== -1) {
        notifyNeedsInput(session, turnText);
        return;
      }
    }
  ...
  function findExistingSessionForItem(recipe, item) {
    ...
    var url = item.url || "";
    var found = null;
    if (!sm.sessions || typeof sm.sessions.forEach !== "function") return null;
  ...
  // Scheduled / automated launch: fetch matching items and start a session for
  // each one that does not already have a session (dedup by recipe + item).
  function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      started.push(startSessionForItem(null, recipe, items[i], args, null, { auto: true }));
    }
    if (started.length > 0) sm.broadcastSessionList();
