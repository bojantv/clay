...
var { listProviderRoutes, routeForId, routeForVendor, knownModelsForProvider } = require("./provider-routes");
...
function attachSessions(ctx) {
  ...
  function resolveModelForVendor(vendor, model, provider) {
    ...
    if (models.length === 0 && provider) models = knownModelsForProvider(provider);
  ...
  function modelsForRoute(route, vendor) {
    ...
    if (route && route.provider) {
      var known = knownModelsForProvider(route.provider);
      if (known.length > list.length) list = known;
    }
  ...
  function modelForHandoff(sourceSession, targetRoute) {
    ...
    return defaultModelForRoute(targetRoute) || currentModel;
  }
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "handoff_session") {
      ...
      var targetRoute = msg.targetRouteId ? routeForId(msg.targetRouteId) : routeForVendor(toVendor);
      ...
      if (handoffTranscript) sourceSession.handoffContext = handoffTranscript;
    ...
    if (msg.type === "set_server_default_model" && msg.model) {
      var serverModelVendor = msg.vendor === "codex" ? "codex" : "claude";
    ...
    if (msg.type === "set_project_default_model" && msg.model) {
      var modelVendor = msg.vendor === "codex" ? "codex" : "claude";
