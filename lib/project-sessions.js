...
function attachSessions(ctx) {
  ...
  function routeForHandoffTarget(toVendor, sourceSession, requestedRouteId) {
    if (requestedRouteId) return routeForId(requestedRouteId);
    if (toVendor !== "github-copilot") return routeForVendor(toVendor);
    var family = familyForModel((sourceSession && sourceSession.model) || "");
  ...
  function modelForHandoff(sourceSession, targetRoute) {
    var currentModel = sourceSession && sourceSession.model && sourceSession.model !== "default" ? sourceSession.model : null;
  ...
  function prepareCodexSessionForView(session) {
    ...
    if (session.storageId && session.storageId !== session.cliSessionId) return;
    var cliSess;
    try { cliSess = require("./cli-sessions"); } catch (e) { return; }
    var home = resolveSessionHome(session);
    var mtime = cliSess.codexRolloutMtime(home, session.cliSessionId, cwd);
    if (!mtime) return;
    ...
    var history = null;
    try { history = cliSess.readCodexHistorySync(home, session.cliSessionId, cwd); } catch (e) { history = null; }
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "list_cli_sessions") {
      var adoptable = sm.listAdoptableCliSessions();
      sendTo(ws, { type: "cli_session_list", sessions: adoptable });
      return true;
    }
    ...
    if (msg.type === "handoff_session") {
      ...
      var targetRoute = routeForHandoffTarget(toVendor, sourceSession, msg.targetRouteId || null);
      ...
      var targetModel = targetRoute ? modelForHandoff(sourceSession, targetRoute) : null;
      ...
      if (handoffTranscript) {
        ...
        sourceSession.handoffContextTurnsRemaining = 4;
      }
      sourceSession.pendingUserMessageQueue = [];
    ...
    if (msg.type === "set_server_follow_up_behavior") {
      var followUpBehavior = msg.behavior === "steer" ? "steer" : "queue";
      if (typeof opts.onSetServerFollowUpBehavior === "function") {
        opts.onSetServerFollowUpBehavior(followUpBehavior);
      }
      sm.followUpBehavior = followUpBehavior;
      send({ type: "follow_up_behavior_changed", behavior: followUpBehavior });
      return true;
    }
    ...
    if (msg.type === "ask_user_response") {
      ...
      if (!pending) {
        ...
        // synchronously blocked on the permission callback. Older Clay builds
        // returned answers via updatedInput, which some agents treated as
        // permission metadata instead of a user answer. Convert it to the same
        // explicit next-turn user message as the MCP path.
        ...
        if (session.isProcessing) {
          sdk.pushMessage(session, nativeAnswerText);
        } else {
          session.isProcessing = true;
          onProcessingChanged();
          session.sentToolResults = {};
          sendToSession(session.localId, { type: "status", status: "processing" });
          if (!session.queryInstance && !session.worker) {
            sdk.startQuery(session, nativeAnswerText, undefined, ensureProjectAccessForSession(session));
          } else {
            sdk.pushMessage(session, nativeAnswerText);
          }
        }
        ...
        pending.resolve({
          behavior: "deny",
          message: "Answer received as the next user message.",
        });
      }
      return true;
    }
