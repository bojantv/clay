      ...
      if (session.isProcessing) {
        ...
        if (!session.destroying) {
          ...
          if (session.taskStopRequested) {
            ...
            // Auto-retry once if the user has auto-continue enabled and this is
            // not a rate-limit-handled stop (rate limit sets rateLimitResetsAt before
            // the finally block, which schedules its own continue).
            if (!session.streamEndedAutoRetryQueued && !session.rateLimitResetsAt && !session.scheduledMessage) {
              var acEnabledForRetry = session.onQueryComplete ||
