...
function createSDKBridge(opts) {
  ...
  // How long to wait for the next event before treating the stream as hung.
  // 3 minutes covers slow tool executions while catching true freezes quickly.
  ...
  async function processQueryStream(session) {
    ...
    var _watchdogTimer = setInterval(function () {
      ...
      if (Date.now() - _lastEventAt >= STREAM_INACTIVITY_TIMEOUT_MS) {
        clearInterval(_watchdogTimer);
        console.warn("[sdk-bridge] Stream inactivity watchdog fired for session " + session.localId +
          " — no event for " + Math.round(STREAM_INACTIVITY_TIMEOUT_MS / 1000) + "s, aborting.");
