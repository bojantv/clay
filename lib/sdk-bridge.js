...
function createSDKBridge(opts) {
  ...
  // schedules a "continue"; the one-shot guards (_transientRetryUsed,
  // streamEndedAutoRetryQueued) re-arm on a successful result, so a session
  // whose turns keep truncating (e.g. a chronically interrupted-like Codex
  // session) would resume → complete → re-arm → truncate → resume forever,
  // burning tokens and constantly reordering the session list. This counter is
  // NOT reset on success — only genuine user input (project-user-message.js)
  // clears it — so an unattended session can resume at most this many times.
  var MAX_CONSECUTIVE_AUTO_RESUMES = 2;
  ...
  var STREAM_FIRST_EVENT_TIMEOUT_MS = 90 * 1000;
  ...
  // 60s is safe here BECAUSE a live generation is never silent this long with
  // no tool running, so shortening it cannot abort a working session — only a
  // genuinely dead one. (The 10-min tool window above is what protects slow
  // tools like Bash, so we deliberately do NOT shorten that.)
  var STREAM_MIDSTREAM_TIMEOUT_MS = 60 * 1000;
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          if (session._watchdogAbort && !session.taskStopRequested && !session.destroying
            ...
            session._transientRetryUsed = true;
            sendAndRecord(session, { type: "thinking_stop" });
            sendAndRecord(session, { type: "info", text: "Response stalled. Resuming\u2026" });
            sendAndRecord(session, { type: "done", code: 0 });
            scheduleInterruptResume(session);
            sm.broadcastSessionList();
            return;
          ...
          if (conflicts.length > 0) {
            ...
            session._transientRetryUsed = true;
            sendAndRecord(session, { type: "thinking_stop" });
            sendAndRecord(session, { type: "info", text: "Connection dropped mid-response. Retrying…" });
            sendAndRecord(session, { type: "done", code: 0 });
            scheduleInterruptResume(session);
            sm.broadcastSessionList();
            return;
