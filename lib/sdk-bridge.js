...
function createSDKBridge(opts) {
  ...
  async function processQueryStream(session) {
    ...
    var _watchdogTimer = setInterval(function () {
      ...
      var hung = _sawAnyEvent
        ? (Date.now() - _lastEventAt >= STREAM_INACTIVITY_TIMEOUT_MS)
        : (Date.now() - _turnStartedAt >= STREAM_FIRST_EVENT_TIMEOUT_MS);
      if (hung) {
        clearInterval(_watchdogTimer);
        console.warn("[sdk-bridge] Stream watchdog fired for session " + session.localId +
          " — sawAnyEvent=" + _sawAnyEvent + ", aborting to auto-resume.");
      ...
      if (session.isProcessing) {
        ...
        if (!session.destroying) {
          ...
          if (session.taskStopRequested) {
            ...
            sendAndRecord(session, { type: "done", code: 1 });
            // Auto-retry once: covers both watchdog-triggered aborts and natural stream ends.
            if (!session.streamEndedAutoRetryQueued && !session.rateLimitResetsAt && !session.scheduledMessage) {
              var acEnabledForRetry = session.streamHungAutoRetryQueued ||
                session.onQueryComplete ||
                (typeof opts.getAutoContinueSetting === "function" && opts.getAutoContinueSetting(session));
              if (acEnabledForRetry) {
                session.streamEndedAutoRetryQueued = true;
                session.streamHungAutoRetryQueued = false;
                if (typeof opts.scheduleMessage === "function") {
                  opts.scheduleMessage(session, "continue", Date.now(), RESUME_AFTER_INTERRUPT_PROMPT, RESUME_DISPLAY_LABEL);
                }
              }
            }
          }
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          if (session._watchdogAbort && !session.taskStopRequested && !session.destroying
              && !session._transientRetryUsed && typeof opts.scheduleMessage === "function") {
          ...
          if (conflicts.length > 0) {
            ...
            sendAndRecord(session, {
              ...
              && !session.taskStopRequested && typeof opts.scheduleMessage === "function") {
