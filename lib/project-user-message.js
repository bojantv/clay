...
function attachUserMessage(ctx) {
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "send_scheduled_now") {
      ...
      // The user explicitly sent the queued message, so treat it as genuine
      // input: reset the auto-resume cap and re-enable lastActivity bumping.
      nowSession._consecutiveAutoResumes = 0;
      nowSession._suppressActivityBump = false;
