...
function attachUserMessage(ctx) {
  ...
  function isClaudeFamilySession(session) {
    if (!session) return false;
    var vendor = session.vendor || "claude";
    if (vendor === "claude") return true;
    if (vendor !== "github-copilot") return false;
    if (session.providerRouteId === "claude-github-copilot") return true;
    var model = session.model || "";
    return model.indexOf("claude-") === 0;
  }

  function shouldQueueDuringProcessing(session) {
    return !!(session && session.isProcessing && isClaudeFamilySession(session));
  }

  function queuePreparedMessage(session, finalText, images) {
    ...
    sendToSession(session.localId, {
      type: "info",
      text: "Message queued until the current response finishes.",
    });
    sm.broadcastSessionList();
  }

  function dispatchPreparedToSdk(session, finalText, images) {
    if (shouldQueueDuringProcessing(session)) {
      queuePreparedMessage(session, finalText, images);
      return;
    }
  ...
  function flushQueuedUserMessage(session) {
    ...
    dispatchPreparedToSdk(session, next.text, next.images);
  }
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (wasProcessing) userMsg2.queuedDuringProcessing = true;
    ...
    if (tabSources.length > 0) {
      ...
      Promise.all(tabPromises).then(function(results) {
        ...
        dispatchPreparedToSdk(session, fullText, msg.images);
      });
    } else {
      dispatchPreparedToSdk(session, fullText, msg.images);
    }
