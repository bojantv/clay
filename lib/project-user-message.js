...
function attachUserMessage(ctx) {
  ...
  function defaultFollowUpIsSteer() {
    return sm.followUpBehavior === "steer";
  }
  ...
  function queuePreparedMessage(session, finalText, images, queueId, displayText, imageCount) {
    if (!session.pendingUserMessageQueue) session.pendingUserMessageQueue = [];
    if (!queueId) queueId = makeQueueId();
    session.pendingUserMessageQueue.push({
      queueId: queueId,
      text: finalText || "",
      images: images || null,
  ...
  function dispatchPreparedToSdk(session, finalText, images, steer, queueId, displayText, imageCount) {
    if (!steer && shouldQueueDuringProcessing(session)) {
      queuePreparedMessage(session, finalText, images, queueId, displayText, imageCount);
      return;
    }
  ...
  function flushQueuedUserMessage(session) {
    ...
    if (!session.pendingUserMessageQueue || session.pendingUserMessageQueue.length === 0) return;
    var next = session.pendingUserMessageQueue.shift();
    sendToSession(session.localId, {
      type: "queued_user_message_removed",
      queueId: next.queueId,
    });
    var queuedHistoryItem = findQueuedHistoryMessage(session, next.queueId);
    if (queuedHistoryItem) {
      sendToSession(session.localId, hydrateImageRefs(queuedHistoryItem));
    }
    dispatchPreparedToSdk(session, next.text, next.images, false, next.queueId, next.displayText, next.imageCount);
  }
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "steer_queued_message") {
      var steerSession = getSessionForWs(ws);
      ...
      dispatchPreparedToSdk(steerSession, steerItem.text, steerItem.images, true, steerItem.queueId, steerItem.displayText, steerItem.imageCount);
      return true;
    }

    if (msg.type === "clear_queued_message") {
      var clearSession = getSessionForWs(ws);
    ...
    var steer = msg.steer === true || (wasProcessing && msg.steer !== false && defaultFollowUpIsSteer());
    var queueId = wasProcessing && !steer ? makeQueueId() : null;
    ...
    if (tabSources.length > 0) {
      ...
      Promise.all(tabPromises).then(function(results) {
        ...
        dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount);
      });
    } else {
      dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount);
    }
