...
function attachUserMessage(ctx) {
  ...
  function queuePreparedMessage(session, finalText, images, queueId, displayText, imageCount, clientMessageId) {
  ...
  function dispatchPreparedToSdk(session, finalText, images, steer, queueId, displayText, imageCount, clientMessageId) {
    if (!steer && shouldQueueDuringProcessing(session)) {
      queuePreparedMessage(session, finalText, images, queueId, displayText, imageCount, clientMessageId);
      return;
    }
  ...
  function flushQueuedUserMessage(session) {
    ...
    dispatchPreparedToSdk(session, next.text, next.images, false, next.queueId, next.displayText, next.imageCount, next.clientMessageId);
  }
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "steer_queued_message") {
      ...
      dispatchPreparedToSdk(steerSession, steerItem.text, steerItem.images, true, steerItem.queueId, steerItem.displayText, steerItem.imageCount, steerItem.clientMessageId);
      return true;
    }
    ...
    if (tabSources.length > 0) {
      ...
      Promise.all(tabPromises).then(function(results) {
        ...
        dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount, clientMessageId);
      });
    } else {
      dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount, clientMessageId);
    }
