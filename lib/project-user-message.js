...
function attachUserMessage(ctx) {
  ...
  function dispatchPreparedToSdk(session, finalText, images, steer, queueId, displayText, imageCount, clientMessageId, pastes) {
    ...
    if (steer && session.isProcessing) {
      if (!queueId) queueId = makeQueueId();
      queuePreparedMessage(session, finalText, images, queueId, displayText, imageCount, clientMessageId, pastes, { front: true, silent: true });
  ...
  function flushQueuedUserMessage(session) {
    ...
    if (queuedHistoryItem && !queuedHistoryItem.steerDuringProcessing) {
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "steer_queued_message") {
      ...
      dispatchPreparedToSdk(steerSession, steerItem.text, steerItem.images, true, steerItem.queueId, steerItem.displayText, steerItem.imageCount, steerItem.clientMessageId, steerItem.pastes);
      return true;
    }
