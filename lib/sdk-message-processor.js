  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (!session._turnSawActivity && isZeroCost && !isLoginPrompt) {
        ...
        sendAndRecord(session, {
          type: "error",
          text: providerDisplayName(session) + " returned an empty response — no output and no token usage. " +
            "The provider session may be wedged or rate-limited. Try resending, or switch provider.",
        });
      }
